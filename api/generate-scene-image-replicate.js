// pages/api/generate-scene-image-replicate.js
...
export default async function handler(req, res) {
  ...
  const REPLICATE_MODEL_VERSION_SCENE = process.env.REPLICATE_MODEL_VERSION_SCENE;
  if (!REPLICATE_API_TOKEN || !REPLICATE_MODEL_VERSION_SCENE) {
    return res.status(500).json({ error: "Missing Replicate API key or scene model version" });
  }
  ...
  try {
    const response = await fetch("https://api.replicate.com/v1/predictions", {
      ...
      body: JSON.stringify({
        version: REPLICATE_MODEL_VERSION_SCENE,
        input: {
          prompt
        }
      })
