...
export default async function handler(req, res) {
	...
	try {
		...
		res.status(200).json({ result: completion.choices?.[0]?.message?.content || "", model: MODEL });
	} catch (error) {
		const code = error?.status || 500;
		const type = error?.status === 429 ? "rate_limit" : (error?.status >= 500 ? "upstream_5xx" : (error?.status >= 400 ? "validation" : "unknown"));
		res.status(code).json({ error: "Failed to generate world.", type, detail: error?.message });
	}
}
